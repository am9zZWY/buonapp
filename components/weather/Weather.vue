  ...
  <span v-if="compact">
    <UPopover v-model:open="isEditMode">
      <template #default="{ open }">
        <button
          :class="[{ 'hover:underline': !disableLastUpdated }]"
          :disabled="disableEdit"
          class="focus:outline-none inline-block text-nowrap text-left"
          @click="open"
        >
          {{ weather.temperature }}°C, {{ weather.weather.toLowerCase() }}<br >in
          {{ weather.location }}
        </button>
      </template>
